...
- The repository now ships `server/python-services/Dockerfile`, `client/web-app/Dockerfile` and a root `docker-compose.yml` covering the whole stack. Section 9 documents them; the one-off `docker run` command is kept as a fallback for testing the Python service in isolation.
...
The repository contains a full Compose stack:

| File | Service | Container port | Default host port |
|---|---|---|---|
| `client/web-app/Dockerfile` + `nginx.conf` | `web` — Nginx serving the Vite build, reverse-proxying `/api` | 80 | 3000 |
| `server/node-api/Dockerfile` | `api` — compiled Express API | 8080 | 8081 |
| `server/python-services/Dockerfile` | `pythonsvc` — combined FastAPI ML service | 5000 | 5000 |

```bash
cp server/node-api/.env.example server/node-api/.env   # fill in secrets
docker compose build
docker compose up -d
./scripts/docker-smoke-test.sh                          # ~45 end-to-end checks
```

Host ports are overridable from the root `.env` via `WEB_HOST_PORT`,
`API_HOST_PORT` and `PY_HOST_PORT`; the container-internal ports never change,
so `PYTHON_API_URL=http://pythonsvc:5000` stays valid. MongoDB is not part of
the stack — the API connects to the company instance through `MONGODB_URI`.
The reliability scoring API (port 8003) is not containerised yet.

See `docs/DOCKER_LOCAL_TESTING.md` for the full testing walkthrough and
`docs/DOCKER_FIX_REPORT.md` for the rationale behind each image.

### 9.2 Testing the Python service in isolation

The following one-off container remains useful for verifying the combined
Python service without the rest of the stack.

It mounts the working tree, installs dependencies into a disposable Python 3.11 container, and leaves the repository unchanged.
