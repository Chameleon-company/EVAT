...
def predict(req: PredictRequest):
    try:
        ...
        if req.ev_make and req.ev_model:
            ev_eff = costComparison.model_runner.get_ev_efficiency(req.ev_make, req.ev_model, req.ev_variant)
        else:
            ev_eff = 0.15

        # Get ICE efficiency from CSV if vehicle selected
        if req.ice_make and req.ice_model:
            ice_eff = costComparison.model_runner.get_ice_efficiency(req.ice_make, req.ice_model, req.ice_variant)
        else:
            ice_eff = 7.0

        result = costComparison.model_runner.predict(
...
def charts(req: PredictRequest):
    try:
        ...
        result = costComparison.model_runner.get_chart_data(
            distance_km=req.distance_km,
            electricity_price_per_kwh=req.electricity_price_per_kwh,
            ice_eff_l_per_100km=ice_eff,
            petrol_price_per_l=req.petrol_price_per_l,
